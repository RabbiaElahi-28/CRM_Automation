"""Sales Backend Approved stage progression."""
...
def test_be_approved_stage(admin_page, be_agent_page, request):
    skip_if_be_agent_unauthenticated(be_agent_page)

    lead_name = setup_be_assigned_lead(admin_page)
    register_test_data(request.node, flow="be_approved", lead_name=lead_name)
