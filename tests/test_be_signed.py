"""Sales Backend Signed stage progression."""
...
def test_be_signed_stage(admin_page, be_agent_page, request):
    skip_if_be_agent_unauthenticated(be_agent_page)

    lead_name = setup_be_assigned_lead(admin_page)
    register_test_data(request.node, flow="be_signed", lead_name=lead_name)
    ...
    run_backend_approved_smoke(be_agent_page, lead_name)
    # run_backend_signed_smoke(be_agent_page, lead_name)
    run_backend_signed_smoke(be_agent_page, lead_name, role="agent")
